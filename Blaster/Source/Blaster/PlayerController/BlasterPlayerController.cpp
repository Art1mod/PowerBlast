...
void ABlasterPlayerController::ClientElimAnnouncement_Implementation(APlayerState* Attacker, APlayerState* Victim) 
{
	...
	if (Attacker && Victim && Self) 
	{
		...
		if (BlasterHUD) 
		{
			...
			{
				BlasterHUD->AddElimAnnouncement(FString("You"), FString("yourself"));
				return;
			}
...
{
	TObjectPtr<ABlasterGameMode> GameMode = Cast<ABlasterGameMode>(UGameplayStatics::GetGameMode(this));
...
void ABlasterPlayerController::ClientJoinMidgame_Implementation(FName StateOfMatch, float Warmup, float Match, float Cooldown, float StartingTime) 
{
	...
	// Initializing HUD for clients if bDelayedStartIsFalse
	if (BlasterHUD && MatchState == MatchState::InProgress) 
	{
...
void ABlasterPlayerController::OnPossess(APawn* InPawn) 
{
	...
	TObjectPtr<ABlasterCharacter> BlasterCharacter = Cast<ABlasterCharacter>(InPawn);
	if (BlasterCharacter)
	{
...
void ABlasterPlayerController::SetHUDMatchCountdown(float CountdownTime) 
{
	...
	if (bHUDValid)
	{
		...
		if (BlasterHUD->CharacterOverlay->UrgentText && Minutes == 0.f && Seconds <= 30.f) 
		{
...
{
	//Making sure that when we in progress match state, we initialize HUD health, score and defeats, etc.	
	...
	if (CharacterOverlay == nullptr)
	{
		if (BlasterHUD && BlasterHUD->CharacterOverlay)
		{
			...
			if (CharacterOverlay)
			{
				...
				TObjectPtr<ABlasterCharacter> BlasterCharacter = Cast<ABlasterCharacter>(GetPawn());
...
void ABlasterPlayerController::SetElimText(FString InText)
{
	...
	if (bHUDValid)
	{
		if (InText.IsEmpty())
		{
			...
			BlasterHUD->CharacterOverlay->DeathMessage->SetText(FText::FromString(TextToDisplay));
		}
		else
		{
		...
		BlasterHUD->CharacterOverlay->DeathMessage->SetVisibility(ESlateVisibility::SelfHitTestInvisible);
	}
}
...
void ABlasterPlayerController::ClearElimText()
{
	...
	if (bHUDValid)
	{
		BlasterHUD->CharacterOverlay->DeathMessage->SetText(FText());
