...
ABlasterCharacter::ABlasterCharacter()
{
	...
	//Ensuring that bones are properly initialized despite the server's view direction.
...
void ABlasterCharacter::BeginPlay()
{
	...
	GetWorldTimerManager().SetTimer(
		DefaultWeaponSpawnTimer,
		this,
		&ABlasterCharacter::DefaultWeaponSpawnTimerFinished,
		DefaultWeaponSpawnDelay
	);
	...
	//Clearing elim message
	if (TObjectPtr<ABlasterPlayerController> BlasterController = Cast<ABlasterPlayerController>(Controller))
	{
		BlasterController->ClearElimText();
	}
...
void ABlasterCharacter::Elim(APlayerController* AttackerController, bool bPlayerLeftGame) 
{
	//difference block
...
void ABlasterCharacter::MulticastElim_Implementation(const FString& AttackerName, bool bPlayerLeftGame) 
{
...
void ABlasterCharacter::ElimTimerFinished() 
{
	...
	if (BlasterGameMode && !bLeftGame)
	{
		...
		if (ABlasterPlayerController* BlasterController = Cast<ABlasterPlayerController>(Controller))
		{
			BlasterController->ClearElimText();
		}
	}
...
void ABlasterCharacter::SetSpawnPoint()
{
	if (HasAuthority() && BlasterPlayerState->GetTeam() != ETeam::ET_NoTeam)
	{
		...
		{
			TObjectPtr<ATeamPlayerStart> ChosenPlayerStart = TeamPlayerStarts[FMath::RandRange(0, TeamPlayerStarts.Num() - 1)];
...
void ABlasterCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent) 
{
	...
	if (TObjectPtr<UEnhancedInputComponent> EnhancedInputComponent = CastChecked<UEnhancedInputComponent>(PlayerInputComponent))
	{
		...
		EnhancedInputComponent->BindAction(UndoAimAction, ETriggerEvent::Triggered, this, &ThisClass::AimButtonReleased);
		EnhancedInputComponent->BindAction(FireAction, ETriggerEvent::Triggered, this, &ThisClass::FireButtonPressed);
		EnhancedInputComponent->BindAction(UndoFireAction, ETriggerEvent::Triggered, this, &ThisClass::FireButtonReleased);
...
void ABlasterCharacter::SpawnDefaultWeapon()
{
	...
	{
		TObjectPtr<AWeapon> StartingWeapon = World->SpawnActor<AWeapon>(DefaultWeaponClass);
...
void ABlasterCharacter::DefaultWeaponSpawnTimerFinished()
{
	SpawnDefaultWeapon();
	
	if (Combat && BlasterPlayerController)
	{
		BlasterPlayerController->SetHUDWeaponType(Combat->GetWeaponTypeDisplayNameText());
	}
}
